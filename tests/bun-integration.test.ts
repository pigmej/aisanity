...
describe('Bun Integration Tests', () => {
  ...
  describe('Bun Project Detection', () => {
    test('detects Bun project with complete structure', () => {
      ...
      fs.writeFileSync(path.join(bunProjectDir, 'package.json'), JSON.stringify({
        name: 'test-bun-api',
        type: 'module',
        scripts: {
      ...
      fs.writeFileSync(path.join(bunProjectDir, 'tsconfig.json'), JSON.stringify({
        compilerOptions: {
      ...
      fs.writeFileSync(path.join(bunProjectDir, 'src', 'index.ts'), `
  ...
  describe('Bun vs Node.js Priority', () => {
    test('Bun detection runs before Node.js detection', () => {
      ...
      fs.writeFileSync(path.join(mixedProjectDir, 'package.json'), JSON.stringify({
        name: 'mixed-project',
        dependencies: {
          'express': '^4.18.0'
        }
